...
instance ITWR_SADRAX(C_Item)
{
	name = "Uralte Steintafel";
	mainflag = ITEM_KAT_NONE;
	flags = ITEM_MISSION | ITEM_MULTI;
	value = value_StonePlateCommon;
	visual = "ItMi_StonePlate_Read_06.3ds";
	material = MAT_LEATHER;
	on_state[0] = use_sadrax;
	scemeName = "MAP";
	inv_rotx = -90;
	inv_roty = 0;
	inv_rotz = 0;
...
func void use_sadrax()
{
	...
	Doc_PrintLine (nDocID, 0, "");
	Doc_PrintLines (nDocID, 0, "Lolroflhar dafrehg dankePB dage");
	Doc_PrintLine (nDocID, 0, "");
	Doc_PrintLines (nDocID, 0, "ragorafag dere rarjig jiun fdasfaskla ukezh djfu bfez beuntzaöoe onebwj");
	Doc_PrintLine (nDocID, 0, "");
	Doc_PrintLines (nDocID, 0, "jMejru -sjArn u huCelHek oTubw WekbEgc IwkjhTe llenEcda  asRefvet eSrbewsavv eOre sw!oiurbf bvhrzj keuhfbczuekuzvb gresu ser bruew vneuibczeb öoegnn");
	Doc_PrintLine (nDocID, 0, "");
	Doc_PrintLines (nDocID, 0, "nuebfb");
	B_Say (self, self, "$CANTREADTHIS");
	Doc_PrintLine (nDocID, 0, "");
	Doc_Show (nDocID);
};
...
instance ITWR_LEARN_1(C_Item)
{
	...
	visual = "ItMi_StonePlate_Read_06.3ds";
	material = MAT_LEATHER;
	on_state[0] = use_learn_1;
	scemeName = "MAP";
	inv_rotx = -90;
	inv_roty = 0;
	inv_rotz = 0;
	description = name;
	text[2] = "Eine  Steintafel";
	text[3] = "";
	text[5] = NAME_Value;
	count[5] = value_StonePlateCommon;
};
...
func void use_learn_1()
{
	...
	Doc_SetMargins (nDocID, -1, 70, 50, 90, 50, 1);
	Doc_PrintLine (nDocID, 0, "");
	Doc_PrintLines (nDocID, 0, "aeruh hbvuegwjkhbeczuke ");
	Doc_PrintLine (nDocID, 0, "");
	Doc_PrintLines (nDocID, 0, "akjhbevbjk kjiwgh kuwge bu6zc kwhg v kwgb zuwg ");
	Doc_PrintLine (nDocID, 0, "");
	Doc_PrintLines (nDocID, 0, "alkewh bvz iwuh ehvihewg öoiw b b guzg6zcg owq ibuzcwoi iuw hbvuzewgf l lkwuehfkjnbuzvhg wioz iubnfioghwzg 87ow ");
	Doc_PrintLine (nDocID, 0, "");
	Doc_PrintLines (nDocID, 0, " iluw");
	B_Say (self, self, "$CANTREADTHIS");
	Doc_PrintLine (nDocID, 0, "");
	Doc_Show (nDocID);
};
...
instance ITMI_SECRETBOTTLE(C_Item)
{
	name = "Flaschenpost";
	mainflag = ITEM_KAT_NONE;
	flags = ITEM_MISSION;
...
func void use_bottles()
{
	B_PlayerFindItem (itwr_secret, 1);
};
