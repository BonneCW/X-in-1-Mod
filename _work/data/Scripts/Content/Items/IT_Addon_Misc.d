...
instance ITMI_RITUALMESSER(C_Item)
{
	name = "Ritualmesser";
	mainflag = ITEM_KAT_NONE;
	flags = ITEM_MISSION;
...
func void ritual()
{
	if (Wld_IsTime (20, 10, 20, 20))
	{
		CreateInvItems (hero, itpo_hero_blood, 1);
		B_Say (self, self, "$Dead");
		CreateInvItems (hero, itmi_ritualmesser, 1);
		self.attribute[ATR_HITPOINTS] = self.attribute[ATR_HITPOINTS] - 10;
	}
	else if (BLOOD_CLANSPENDE == TRUE)
	{
		CreateInvItems (hero, itpo_hero_blood, 1);
		B_Say (self, self, "$Dead");
		CreateInvItems (hero, itmi_ritualmesser, 1);
...
instance ITMI_ARDOCSPAKET(C_Item)
{
	name = "Ardocs Paket";
	mainflag = ITEM_KAT_NONE;
	flags = ITEM_MISSION;
...
instance ITMI_SKULLGOLD(C_Item)
{
	name = "Schädel";
	mainflag = ITEM_KAT_NONE;
	flags = ITEM_MISSION;
...
instance ITMI_RAVENAMOR(C_Item)
{
	name = "Ravens Rüstung";
	mainflag = ITEM_KAT_NONE;
	flags = ITEM_MISSION;
	...
	material = MAT_LEATHER;
	description = name;
	text[2] = "Das ist Ravens Rüsteung";
	text[3] = "Sie ist beschädigt";
	text[4] = "Ein fähiger Schmied kann sie reparieren.";
	text[5] = NAME_Value;
	count[5] = value;
};
