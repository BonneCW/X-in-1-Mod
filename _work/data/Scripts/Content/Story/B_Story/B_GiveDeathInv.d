...
func void B_GiveDeathInv (var C_NPC slf)
{
	...
	// FUNC
	if ((slf.aivar[AIV_MM_REAL_ID] == ID_SHEEP) && Wld_IsTime (3, 10, 3, 20))
	&& (SC_IST_EIN_VAMPIR == TRUE)
	{
		CreateInvItems (slf, itpo_sheep_blood, 1);
	};
	...
	if (PLAYER_TALENT_TAKEANIMALTROPHY[TROPHY_Claws] == TRUE)
	{
		...
		if (slf.aivar[AIV_MM_REAL_ID] == ID_Alligator)		{	CreateInvItems (slf, ItAt_Claw		, 4);	};
	...
	if (PLAYER_TALENT_TAKEANIMALTROPHY[TROPHY_Fur] == TRUE)
	{
		...
		if (Hlp_GetInstanceID (self) == Hlp_GetInstanceID (BlackWolf))
		{
			CreateInvItems (slf, itat_blackfur, 1);
		};
	};
	...
	if (slf.aivar[AIV_MM_REAL_ID] == ID_MEATBUG)		{	CreateInvItems (slf, ItAt_MeatbugFlesh, 1);		}
	  ...
	  CreateInvItems (slf, ItMi_Feuerstein, 1);
	}
	...
	&& (SC_IST_EIN_VAMPIR == TRUE)
	{
		CreateInvItems (self, itpo_blood, 2);
	}
	...
	else if (Hlp_GetInstanceID (self) == Hlp_GetInstanceID (fat_blattcrawler))
	{
		CreateInvItems (self, itwr_drusus_tafel, 1);
	};	
