...
FUNC VOID B_ENTER_OLDWORLD_Kapitel_2 ()
{
  if (EnterOW_Kapitel2 == FALSE)
    {
      // ------ neue NPCs im Minental
      Wld_InsertNPC(B5_90020_Helmchen_OW, "OW_STAND_JERGAN");
...
FUNC VOID B_ENTER_OLDWORLD_Kapitel_3 ()
{
	...
	{
		// ------ Gilden-Attitüden ändern ------
	  Wld_AssignRoomToGuild("DT1", GIL_DMT); //Vorsichthalber, falls Helmchen den Turm betreten hat
	  Wld_AssignRoomToGuild("DT2", GIL_DMT);
		...
		// ------ TAs ändern ------
		B_StartOtherRoutine(B5_90020_Helmchen_OW, "OldCamp");
...
FUNC VOID B_ENTER_OLDWORLD_Kapitel_4 ()
{
	if (EnterOW_Kapitel4 == FALSE)	// Inserten der Drachenjäger wenn Drachenjagd eröffnet
		...
		// Dunkle Magie
		Wld_InsertNpc(VLK_574_Mud, "OC_GUARD_ENTRANCE");
		Wld_InsertItem(ItWr_Urkunde, "LOCATION_19_03_ROOM6_BARRELCHAMBER2");
		Wld_InsertItem(ITAR_Orebaron_Addon, "LOCATION_19_03_ROOM6_BARRELCHAMBER2");
...
FUNC VOID B_ENTER_OLDWORLD_Kapitel_5 ()
{
	if  (EnterOW_Kapitel5 == FALSE)
		...
		// ------ Respawn ------
		  if (hero.guild == GIL_BAD)
		    {
		      Wld_InsertNpc (Bad_9006_Krieg, "OC1");
		    };			
		// ------ Immortal-Flags löschen ------
	...
	if (Npc_KnowsInfo(hero, Nur_Shak_Waffenstillstand))
	  {
	    B_RemoveNpc(Orc_9011_Nur_Shak);
	  };
};
