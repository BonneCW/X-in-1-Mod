...
INSTANCE PC_PotionAlchemy_Uriziel (C_Info)
{
	npc				= PC_Hero;
	nr				= 999;
	condition		= PC_PotionAlchemy_Uriziel_Condition;
	information		= PC_PotionAlchemy_Uriziel_Info;
	permanent		= TRUE;
	description		= "Uriziel und die Tränen Innos verbinden"; 
};

FUNC INT PC_PotionAlchemy_Uriziel_Condition ()
{
	if (PLAYER_MOBSI_PRODUCTION	==	MOBSI_POTIONALCHEMY)
	&& (BoozeStart == FALSE)
	&& (TabakStart == FALSE)
	...
	&& (SpecialStart == FALSE)
        && (FINALKNOWING == TRUE)
 	&& ((Npc_HasItems (self, ItPo_PotionOfDeath_01_Mis) >= 1)
        || (Npc_HasItems (self, ItPo_PotionOfDeath_02_Mis) >= 1))
  	&& (Npc_HasItems (self, ItMw_uriziel) >= 1)
	{	
		return TRUE;
	};
};

FUNC VOID PC_PotionAlchemy_Uriziel_Info()
{
	CreateInvItems (self, ItMw_uriziel2,1);
        Npc_RemoveInvItems (hero,ItMw_uriziel,1);
...
instance PC_ITPO_BLUTGEMISCH(C_Info)
{
	nr = 2;
	npc = PC_Hero;
	condition = pc_itpo_blutgemisch_condition;
	information = pc_itpo_blutgemisch_info;
	permanent = TRUE;
	description = "Blutgemisch (Mein Blut und Schafsblut)";
};


func int pc_itpo_blutgemisch_condition()
{
	if ((PLAYER_MOBSI_PRODUCTION == MOBSI_PotionAlchemy) && (SpecialStart == TRUE) && Npc_KnowsInfo (other, dia_ardok_kahr_blutritual_2))
	{
		return TRUE;
	};
};

func void pc_itpo_blutgemisch_info()
{
	if ((Npc_HasItems (hero, itpo_hero_blood) >= 1) && (Npc_HasItems (hero, itpo_sheep_blood) >= 1))
	{
		Npc_RemoveInvItems (hero, itpo_hero_blood, 1);
		Npc_RemoveInvItems (hero, itpo_sheep_blood, 1);
		CreateInvItems (hero, itpo_blood_mix, 1);
		Print (PRINT_AlchemySuccess);
	}
	else
	{
...
instance PC_ITPO_GEMISCH(C_Info)
{
	nr = 2;
	npc = PC_Hero;
	condition = pc_itpo_gemisch_condition;
	information = pc_itpo_gemisch_info;
	permanent = TRUE;
	description = "Gemisch mit Grog vermischen";
};


func int pc_itpo_gemisch_condition()
{
	if ((PLAYER_MOBSI_PRODUCTION == MOBSI_PotionAlchemy) && (SpecialStart == TRUE) && Npc_KnowsInfo (other, dia_drusus_eremit_2))
	{
		return TRUE;
	};
};

func void pc_itpo_gemisch_info()
{
	if ((Npc_HasItems (hero, ItFo_Addon_Grog) >= 1) && (Npc_HasItems (hero, itpo_gemisch) >= 1))
	{
		Npc_RemoveInvItems (hero, ItFo_Addon_Grog, 1);
		Npc_RemoveInvItems (hero, itpo_gemisch, 1);
		CreateInvItems (hero, itpo_gemisch_2, 1);
		Print (PRINT_AlchemySuccess);
	}
	else
	{
...
instance PC_ITPO_BLOOD_ERSATZ(C_Info)
{
	nr = 2;
	npc = PC_Hero;
	condition = pc_itpo_blood_ersatz_condition;
	information = pc_itpo_blood_ersatz_info;
	permanent = TRUE;
	description = "Blutersatz herstellen";
};


func int pc_itpo_blood_ersatz_condition()
{
	if ((PLAYER_MOBSI_PRODUCTION == MOBSI_PotionAlchemy) && (SpecialStart == TRUE) && (SC_CAN_VAMPERSATZ == TRUE))
	{
		return TRUE;
	};
};

func void pc_itpo_blood_ersatz_info()
{
	if ((Npc_HasItems (hero, ItPl_Mana_Herb_02) >= 1) && (Npc_HasItems (hero, ItFo_Milk) >= 1) && (Npc_HasItems (hero, ItPl_Blueplant) >= 1))
	{
		Npc_RemoveInvItems (hero, ItPl_Mana_Herb_02, 1);
		Npc_RemoveInvItems (hero, ItFo_Milk, 1);
		Npc_RemoveInvItems (hero, ItPl_Blueplant, 1);
		CreateInvItems (hero, itpo_blood_ersatz, 1);
		Print (PRINT_AlchemySuccess);
	}
	else
	{
...
instance PC_ITPO_BLOODDRINK(C_Info)
{
	nr = 2;
	npc = PC_Hero;
	condition = pc_itpo_blooddrink_condition;
	information = pc_itpo_blooddrink_info;
	permanent = TRUE;
	description = "Drachenblutgebräu herstellen";
};


func int pc_itpo_blooddrink_condition()
{
	if ((PLAYER_MOBSI_PRODUCTION == MOBSI_PotionAlchemy) && (SpecialStart == TRUE) && (SC_LEARN_BLOODDRINK == TRUE) && (SC_IST_EIN_VAMPIR == TRUE))
	{
		return TRUE;
	};
};

func void pc_itpo_blooddrink_info()
{
	if ((Npc_HasItems (hero, ItAt_DragonBlood) >= 3) && (Npc_HasItems (hero, ItAt_SharkTeeth) >= 1) && (Npc_HasItems (hero, ItPl_Speed_Herb_01) >= 1))
	{
		Npc_RemoveInvItems (hero, ItAt_DragonBlood, 3);
...
instance PC_ITPO_HEILTRANK_BLOOD(C_Info)
{
	nr = 2;
	npc = PC_Hero;
	condition = pc_itpo_heiltrank_blood_condition;
	information = pc_itpo_heiltrank_blood_info;
	permanent = TRUE;
	description = "Heilung gegen Vampirismus herstellen";
};


func int pc_itpo_heiltrank_blood_condition()
{
	if ((PLAYER_MOBSI_PRODUCTION == MOBSI_PotionAlchemy) && (SpecialStart == TRUE) && (SC_CAN_HEAL_VAMPI == TRUE))
	{
		return TRUE;
	};
};

func void pc_itpo_heiltrank_blood_info()
{
	if ((Npc_HasItems (hero, ItAt_CrawlerMandibles) >= 2) && (Npc_HasItems (hero, ItAt_DrgSnapperHorn) >= 1) && (Npc_HasItems (hero, ItFo_Milk) >= 2) && (Npc_HasItems (hero, ItPl_Mana_Herb_03) >= 5) && (Npc_HasItems (hero, ItAt_DemonHeart) >= 1))
	{
		...
		CreateInvItems (hero, itpo_heiltrank_blood, 1);
		Print (PRINT_AlchemySuccess);
	}
	else
	{
...
instance PC_UEBERZUG(C_Info)
{
	nr = 2;
	npc = PC_Hero;
	condition = pc_ueberzug_condition;
	information = pc_ueberzug_info;
	permanent = TRUE;
	description = "Überzug herstellen";
};


func int pc_ueberzug_condition()
{
	if ((PLAYER_MOBSI_PRODUCTION == MOBSI_PotionAlchemy) && (SpecialStart == TRUE) && (SC_CAN_TIMESWORDUEBERZUG == TRUE))
	{
		return TRUE;
	};
};

func void pc_ueberzug_info()
{
	if ((Npc_HasItems (hero, ItMi_Zeitspalt_Addon) >= 1) && (Npc_HasItems (hero, ItMi_RuneBlank) >= 1) && (Npc_HasItems (hero, ItPl_Perm_Herb) >= 1) && (Npc_HasItems (hero, ItPl_Strength_Herb_01) >= 1))
	{
		Npc_RemoveInvItems (hero, ItMi_Zeitspalt_Addon, 1);
		Npc_RemoveInvItems (hero, ItMi_RuneBlank, 1);
		Npc_RemoveInvItems (hero, ItPl_Perm_Herb, 1);
		Npc_RemoveInvItems (hero, ItPl_Strength_Herb_01, 1);
		CreateInvItems (hero, itpo_ueberzug, 1);
		Print (PRINT_AlchemySuccess);
	}
	else
	{
