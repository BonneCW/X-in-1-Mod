...
func void B_CreateAmbientInv_Bad (var int InventorySet)
{
  if (InventorySet == 0)
    {
  ...
  else if (InventorySet == 7) //Spezial: Der letzte Dunkelkrieger
    {
      CreateInvItems(self, ItPo_MegaDrink, 1);
...
func void B_CreateAmbientInv (var C_NPC slf)
{
	...
	if (Npc_HasItems(self, ItPo_Blood) > 1)
	{
	...
	else if (slf.guild == GIL_BAD) { B_CreateAmbientInv_Bad(Zufall); }
