...
FUNC VOID DIA_Alwin_Endlos_Info()
{
	...
	if ((ALWINSSHEEP == FALSE) && (Npc_KnowsInfo (other, dia_alvin_shaf) == FALSE))
	{
		AI_Output (self, other, "Dia_Alvin_sheep_00");	//Mir ist eins entlaufen.
		AI_Output (other, self, "Dia_Alvin_sheep_01");	//Entlaufen?
		AI_Output (self, other, "Dia_Alvin_sheep_02");	//Ja, eines Tages war es nicht mehr da. Nicht das ich ein Problem damit hätte, aber ich könnte von der Milz eine Menge Ärger kriegen.
		AI_Output (self, other, "Dia_Alvin_sheep_03");	//Könntest du vielleicht nach dem Schaf suchen? Es muss hier irgendwo im Hafenviertel sein.
		AI_Output (other, self, "Dia_Alvin_sheep_04");	//Ich kann nichts versprechen.
		ALWINSSHEEP = TRUE;
		Log_CreateTopic (ALWINSSCHAF, LOG_MISSION);
		Log_SetTopicStatus (ALWINSSCHAF, LOG_Running);
		B_LogEntry (ALWINSSCHAF, "Alwin hat ein Schaf verloren. Er hat mich gebeten nach den Schaf ausschau zu halten.");
	}
	else if ((MIS_AttackFellan != LOG_SUCCESS) && (Npc_IsDead (Fellan) == FALSE))
	{
...
instance DIA_ALVIN_SHAF(C_Info)
{
	npc = Vlk_424_Alwin;
	nr = 123;
	condition = dia_alvin_shaf_condition;
	information = dia_alvin_shaf_info;
	important = FALSE;
	permanent = TRUE;
	description = "Ich habe ein Schaf gefunden";
};


func int dia_alvin_shaf_condition()
{
	if (ALWINLOST == TRUE)
	{
		return TRUE;
...
func void dia_alvin_shaf_info()
{
	AI_Output (other, self, "Dia_Alvin_shaf_00");	//Ich habe ein Schaf gefunden!
	if (Npc_GetDistToNpc (self, follow_sheep_alwin) < 500)
	{
		follow_sheep_alwin.aivar[AIV_PARTYMEMBER] = FALSE;
		follow_sheep_alwin.aivar[AIV_TAPOSITION] = TRUE;
		AI_GotoWP (follow_sheep_alwin, "NW_CITY_SHEEP_SPAWN_01");
		follow_sheep_alwin.wp = "NW_CITY_SHEEP_SPAWN_01";
		follow_sheep_alwin.start_aistate = ZS_MM_AllScheduler;
		AI_Output (self, other, "Dia_Alvin_shaf_01");	//Das ist das entlaufene Schaf! Danke, du hast mir eine Menge Ärger erspart. Hier, nimm die Belohnung!
		CreateInvItems (self, ItMi_Gold, 20);
		B_GiveInvItems (self, other, ItMi_Gold, 20);
		CreateInvItems (self, ItFo_Schafswurst, 1);
		B_GiveInvItems (self, other, ItFo_Schafswurst, 1);
		Log_SetTopicStatus (ALWINSSCHAF, LOG_SUCCESS);
		B_GivePlayerXP (200);
		AI_StopProcessInfos (self);
		ALWINLOST = FALSE;
	}
	...
	{
		AI_Output (self, other, "Dia_Alvin_shaf_02");	//Und wo?
		AI_StopProcessInfos (self);
	};
};
