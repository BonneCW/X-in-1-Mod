...
instance DIA_Pedro_Mythrilpacket		(C_INFO)
{
	npc			 = 	NOV_600_Pedro;
	nr			 = 	90;
	condition	 = 	DIA_Pedro_Mythrilpacket_Condition;
	information	 = 	DIA_Pedro_Mythrilpacket_Info;
	permanent	 = 	TRUE;
	description	 = 	"Ich bringe dir das Mythril-Päckchen von Reton.";
};

func int DIA_Pedro_Mythrilpacket_Condition ()
{
if ( Npc_HasItems(other,ItMi_MPacket) >= 1)
{  		
	return TRUE;
};
};

func void DIA_Pedro_Mythrilpacket_Info ()
{
	AI_Output (other, self, "DIA_Pedro_Mythrilpacket_15_00"); //Ich bringe dir das Mythril-Päckchen von Reton.
	AI_Output (self, other, "DIA_Pedro_Mythrilpacket_09_01"); //Ah, sehr gut.
	AI_Output (self, other, "DIA_Pedro_Mythrilpacket_09_02"); //(gierig) Gib her. Bezahlt ist das Ding schon.

	B_GiveInvItems ( other, self, ItMi_MPacket, 1);
	PTOPEDRO = TRUE;
}; 
