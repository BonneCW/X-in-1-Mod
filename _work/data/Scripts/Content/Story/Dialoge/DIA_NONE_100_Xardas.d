...
{	
	...
	if (Mod_DGDH_An == 1)
	{
		MEM_RemoveVob("ITWR_UBUCH1");
		MEM_RemoveVob("ITWR_UBUCH2");
	};
...
func void DIA_Xardas_RITUALREQUEST_Info ()
{
	...
	|| (hero.guild == GIL_DJG)
	|| (hero.guild == GIL_PAL)
	|| ((hero.guild == GIL_Bad) && (SC_isDarkMage || SC_isDarkWarrior))
        || (hero.guild == GIL_OUT)
        || (hero.guild == GIL_BAU)
	|| (hero.guild == GIL_NONE)
	{
...
instance DIA_XARDAS_ENDOFDAYS_END(C_Info)
{
	npc = NONE_100_Xardas;
	nr = 997;
	condition = dia_xardas_endofdays_end_condition;
	information = dia_xardas_endofdays_end_info;
	important = TRUE;
	permanent = FALSE;
};


func int dia_xardas_endofdays_end_condition()
{
	if (XARDAS_LABER_LOS == TRUE)
	{
		return TRUE;
	};
};

func void dia_xardas_endofdays_end_info()
{
	AI_PlayAni (self, "T_MAGRUN_2_HEASHOOT");
	Wld_PlayEffect ("spellFX_MassDeath", self, self, 0, 0, 0, FALSE);
	AI_PlayAni (self, "T_HEASHOOT_2_STAND");
	AI_Output (self, other, "Dia_Xardas_Endofdays_end_00");	//Halt!
	if (KLAUE_DRUSUS == TRUE)
	{
		vam_90108_drusus_nw.flags = 0;
		vam_90107_cortus_nw.flags = 0;
		vam_90109_kira_nw.flags = 0;
		Npc_ChangeAttribute (vam_90108_drusus_nw, ATR_HITPOINTS, -5000);
		Npc_ChangeAttribute (vam_90107_cortus_nw, ATR_HITPOINTS, -5000);
		Npc_ChangeAttribute (vam_90109_kira_nw, ATR_HITPOINTS, -5000);
	}
	else
	{
		VLK_419_Buerger.flags = 0;
		sld_40010_berox.flags = 0;
		vlk_40001_lea.flags = 0;
		Npc_ChangeAttribute (VLK_419_Buerger, ATR_HITPOINTS, -5000);
		Npc_ChangeAttribute (sld_40010_berox, ATR_HITPOINTS, -5000);
		Npc_ChangeAttribute (vlk_40001_lea, ATR_HITPOINTS, -5000);
	};
	AI_Output (self, other, "Dia_Xardas_Endofdays_end_01");	//Was fällt dir ein mit diesem Abschaum gemeinsame Sache zu machen?
	AI_Output (other, self, "Dia_Xardas_Endofdays_end_02");	//Ich...
	AI_Output (self, other, "Dia_Xardas_Endofdays_end_03");	//Spar dir deine Ausreden! Eigenlich sollte ich dich auf der Stelle töten!
	AI_Output (self, other, "Dia_Xardas_Endofdays_end_04");	//Ich warne dich: Noch einmal so eine Dummheit und du bist dran!
	B_GivePlayerXP (800);
	if (ARDOC_KLAUE == TRUE)
	{
		AI_Output (other, self, "Dia_Xardas_Endofdays_end_05");	//Ich kann nichts dafür, Ardoc ..äh.. Rodan hat mich reingelegt.
		AI_Output (self, other, "Dia_Xardas_Endofdays_end_06");	//Rodan war schon immer auf seinen Vorteil aus.
		AI_Output (other, self, "Dia_Xardas_Endofdays_end_07");	//Du kennst ihn von früher?
		AI_Output (self, other, "Dia_Xardas_Endofdays_end_08");	//(zu sich selbst) Warum hab ich das nur erwähnt?
		AI_Output (self, other, "Dia_Xardas_Endofdays_end_09");	//Geh jetzt und erfülle deine Aufgaben!
	};
	ALLES_ZU_ENDE = TRUE;
	if (Xardas_GoesToRitualInnosEye == FALSE)
	{
		Npc_ExchangeRoutine (NONE_100_Xardas, "start");
	}
	else
	{
		Npc_ExchangeRoutine (NONE_100_Xardas, "ritualInnosEyeRepair");
	};
	AI_StopProcessInfos (self);
};