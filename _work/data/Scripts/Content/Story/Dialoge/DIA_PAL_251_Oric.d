...
instance DIA_Oric_Nordmann (C_INFO)
{
	npc		= PAL_251_Oric;
	nr		= 4;
	condition	= DIA_Oric_Nordmann_Condition;
	information	= DIA_Oric_Nordmann_Info;
	permanent	= FALSE;
	description	= "Man sagt, dass du dich länger mit dem Nordmarer unterhalten hast, der vor kurzem hier war.";
};

func int DIA_Oric_Nordmann_Condition ()
{
	if Npc_KnowsInfo (other, DIA_Garond_Nordmann)
	|| Npc_KnowsInfo (other, DIA_Tandor_Nordmann)
	{
		return 1;
	};
};
func void DIA_Oric_Nordmann_Info ()
{
	AI_Output (other, self, "DIA_Oric_Nordmann_15_00"); //Man sagt, dass du dich länger mit dem Nordmarer unterhalten hast, der vor kurzem hier war.
	AI_Output (self, other, "DIA_Oric_Nordmann_11_01"); //In der Tat, wir haben uns etwas unterhalten.
	AI_Output (other, self, "DIA_Oric_Nordmann_15_02"); //Worüber habt ihr gesprochen?
	AI_Output (self, other, "DIA_Oric_Nordmann_11_03"); //Er hat mich viel über die Orks gefragt. Er scheint sich im Kampf mit ihnen schon auszukennen.
	AI_Output (self, other, "DIA_Oric_Nordmann_11_04"); //Er hat mich viel zum Hinterland und den Nachschubwegen der Orks gefragt.
	AI_Output (self, other, "DIA_Oric_Nordmann_11_05"); //Darüber konnte ich ihm allerdings nur wenig sagen, da wir seit dem Angriff der Drachen in der Burg festsitzen und nur sehr spärliche Informationen aus dem Hinterland bekommen.
	AI_Output (other, self, "DIA_Oric_Nordmann_15_06"); //Weißt du, wo er sich jetzt befindet?
	AI_Output (self, other, "DIA_Oric_Nordmann_11_07"); //Nein, leider nicht.
	AI_Output (self, other, "DIA_Oric_Nordmann_11_08"); //Ich weiß nur, dass er den Orks in den Rücken fallen, ihren Nachschub abschneiden wollte.
	AI_Output (other, self, "DIA_Oric_Nordmann_15_09"); //Dann werde ich mich auf die Suche nach ihm machen.
	AI_Output (self, other, "DIA_Oric_Nordmann_11_10"); //Tu das. Möge Innos' Licht deinen Weg erleuchten.

	B_GivePlayerXP	(100);

	B_LogEntry	(TOPIC_DGDH_NORDMANN, "Der Nordmarer ist nicht mehr in der Burg. Oric hat mir berichtet, dass er im Hinterland Orks jagen wollte. Warum hat mir Udar das nicht gleich gesagt?");
