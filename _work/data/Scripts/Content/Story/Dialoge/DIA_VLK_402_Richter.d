...
FUNC INT DIA_Richter_RichtersLakai_Condition()
{
	...
	if ((Npc_KnowsInfo (other,DIA_Richter_Hello)) && (Kapitel == 5))
	&& (hero.guild == GIL_BAU)
	{
		SCIstRichtersLakai = TRUE;
...
INSTANCE DIA_Richter_PermissionForShip_Dark (C_INFO)
{
	npc			= Vlk_402_Richter;
	nr			= 2;
	condition	= DIA_Richter_PermissionForShip_Dark_Condition;
	information	= DIA_Richter_PermissionForShip_Dark_Info;

	description =	"Gib mir eine Ermächtigung für das Schiff der Paladine.";
};                       

FUNC INT DIA_Richter_PermissionForShip_Dark_Condition()
{
  if ((other.guild == GIL_Bad) && (Npc_HasItems(other, ItWr_ForgedShipLetter_Mis)<1) && Larius_abgesetzt)
	{
		return TRUE;
	};	
};

FUNC VOID DIA_Richter_PermissionForShip_Dark_Info ()
{	
	AI_Output (other,self ,"DIA_Richter_PermissionForShip_Dark_15_00"); //Gib mir eine Ermächtigung für das Schiff der Paladine.
	AI_Output (self ,other,"DIA_Richter_PermissionForShip_Dark_10_01"); //(Lacht) Du bist ja wohl nicht ganz bei Trost. Was soll ich tun?
	AI_Output (other,self ,"DIA_Richter_PermissionForShip_Dark_15_02"); //Du hast mich schon verstanden. Gib mir die Ermächtigung oder du wirst es bereuen.
	AI_Output (self ,other,"DIA_Richter_PermissionForShip_Dark_10_03"); //(Brüllt) Warum sollte ich es bereuen? Ich bin der Richter!
	AI_Output (other,self ,"DIA_Richter_PermissionForShip_Dark_15_04"); //Und ich bin der Statthalter!
	AI_Output (self ,other,"DIA_Richter_PermissionForShip_Dark_10_05"); //(Brüllt) Arrh.
	AI_Output (self ,other,"DIA_Richter_PermissionForShip_Dark_10_06"); //Das wird dir noch Leid tun. Mich erpresst man nicht ungestraft.
	AI_Output (self ,other,"DIA_Richter_PermissionForShip_Dark_10_07"); //Hier hast du die Ermächtigung. Nun geh. Ich werde später mit dir abrechnen.
	CreateInvItems (self,ItWr_ForgedShipLetter_Mis,1);
	B_GiveInvItems (self,other,ItWr_ForgedShipLetter_Mis,1);
	B_LogEntry (TOPIC_Ship,"Mit dem Ermächtigungsschreiben des Richters sollte es für mich kein Problem mehr sein, das Schiff in meinen Besitz zu bringen."); 
