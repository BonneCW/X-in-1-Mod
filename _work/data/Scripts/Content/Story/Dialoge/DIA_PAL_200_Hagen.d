...
func void DIA_Addon_Lord_Hagen_Ornament_Info ()
{
	...
	AI_Output (self, other, "Dia_Lord_hagen_Ornament_hallo_00");	//Ein Bürger Namens Ardoc zeigte interesse dafür, sodass wir es ihm verkauft haben.
...
//Statthalter werden
instance Hagen_Statthalter (C_Info)
{
  npc = PAL_200_Hagen;
  nr = 6;
  condition = Hagen_Statthalter_Condition;
  information = Hagen_Statthalter_Info;
  description = "Ich soll hier Statthalter werden.";
};

func int Hagen_Statthalter_Condition ()
{
  if ((Npc_KnowsInfo(other, Larius_Statthalter) == TRUE) && (Npc_HasItems(other, ItWr_Stadthalter2) > 0))
    {
      return true;
    };
  return false;
...
func void Hagen_Statthalter_Info ()
{
  AI_Output(other, self, "Hagen_Statthalter_01"); //Ich soll hier Statthalter werden.
  AI_Output(self, other, "Hagen_Statthalter_02"); //Wer sagt das?
  AI_Output(other, self, "Hagen_Statthalter_03"); //Ich habe hier einen Brief, den Garond unterzeichnet hat.
  B_GiveInvItems(other, self, ItWr_Stadthalter2, 1);
  B_UseFakeScroll();
  AI_Output(self, other, "Hagen_Statthalter_04"); //Mhh, tatsächlich, es regiert also wieder ein Erzbaron über Khorinis.
  AI_Output(self, other, "Hagen_Statthalter_05"); //Aber warum zeigst du mir das?
  AI_Output(other, self, "Hagen_Statthalter_06"); //Garond meinte, du könntest mir helfen, Larius akzeptiert dieses Schreiben nicht.
  AI_Output(self, other, "Hagen_Statthalter_07"); //Sag ihm, es ist rechtens, wenn er es dennoch nicht anerkennt, so ist das ein Verstoß gegen das Gesetz, da wir im Krieg sind, wird er eine schwere Strafe ertragen müssen.
  AI_Output(self, other, "Hagen_Statthalter_08"); //Den Brief muss ich jedoch behalten, in den falschen Händen, könnte er großen Schaden anrichten, ich werde ihn vernichten.
  B_GivePlayerXP(XP_Ambient);
  B_LogEntry(Statthalter, "Lord Hagen hat meinen Anspruch anerkannt, er wird mir helfen, Larius von MEINEM Platz zu entfernen, wenn er sich weiter weigert.");
};

instance Hagen_Verhandlungen (C_Info)
{
  npc = PAL_200_Hagen;
  nr = 7;
  condition = Hagen_Verhandlungen_Condition;
  information = Hagen_Verhandlungen_Info;
  description = "Gute Neuigkeiten, die Orks sind bereit zu verhandeln.";
};

func int Hagen_Verhandlungen_Condition ()
{
  return Npc_KnowsInfo(other, Nur_Shak_Waffenstillstand);
};

func void Hagen_Verhandlungen_Info ()
{
  AI_Output(other, self, "Hagen_Verhandlungen_01"); //Gute Neuigkeiten, die Orks sind bereit zu verhandeln.
  AI_Output(self, other, "Hagen_Verhandlungen_02"); //Was? Wie hast du das denn geschafft? Die Orks wollen also verhandeln, ich wußte ich kann mich auf Garond verlassen.
  AI_Output(other, self, "Hagen_Verhandlungen_03"); //Oh, die Orks gehen davon aus, dass ihr verhandeln wollt ...
  AI_Output(self, other, "Hagen_Verhandlungen_04"); //(brüllt) WAS? Was hast du getan? Die Paladine des Königs, die Diener Innos sollen mit den Kreaturen Beliars verhandeln wollen? Bist du des Wahnsinns?
  AI_Output(other, self, "Hagen_Verhandlungen_05"); //Komm wieder auf den Teppich, ohne mich wären deine Leute im Minental verloren gewesen! Die Orks hätten die Burg ohne weiteres gestürmt!
  AI_Output(self, other, "Hagen_Verhandlungen_06"); //Du wagst es? Aber jetzt ist es zu spät, ich werde alle meine Leute brauchen, versuche deinen Fehler wenigstens zum Teil wieder gut zu machen in dem du den Rittern, die das Schiff bewachen, diese neuen Befehle bringst!
  CreateInvItems(self, ItWr_ForgedShipLetter_Mis, 1);
  B_GiveInvItems(self, other, ItWr_ForgedShipLetter_Mis, 1);
  AI_Output(self, other, "Hagen_Verhandlungen_07"); //Damit bist du für das Schiff verantwortlich! Pass gut darauf auf, wenn wir wiederkommen, wollen wir keine Überraschung erleben!
  Log_SetTopicStatus(Verhandlungen, LOG_SUCCESS);
  B_GivePlayerXP(XP_Ambient);
};
