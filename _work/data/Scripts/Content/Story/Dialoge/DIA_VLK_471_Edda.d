...
instance DIA_EDDA_BRATEN(C_Info)
{
	npc = VLK_471_Edda;
	nr = 3;
	condition = dia_edda_braten_condition;
	information = dia_edda_braten_info;
	permanent = TRUE;
	important = FALSE;
	description = "Kannst du mir dieses Fleisch braten?";
};


func int dia_edda_braten_condition()
{
	if (Npc_HasItems (other, ItFoMuttonRaw) > 0)
	{
		return TRUE;
	}
	else
...
func void dia_edda_braten_info()
{
	var int anz_brat_dm;

	AI_Output (other, self, "DIA_Edda_Brat_01");	//Kannst du mir dieses Fleisch braten?

	if (EDDA_BRAT_DAY >= Wld_GetDay())
	{
		AI_Output (self, other, "DIA_Edda_Brat_02");	//Komm morgen wieder!
	}
	else
	{
		AI_Output (self, other, "DIA_Edda_Brat_03");	//Klar, hier!
		anz_brat_dm = Npc_HasItems (other, ItFoMuttonRaw);
		Npc_RemoveInvItems (other, ItFoMuttonRaw, anz_brat_dm);
		CreateInvItems (other, ItFoMutton, anz_brat_dm);

		var string FleischLieferung;

		Fleischlieferung = ConcatStrings(IntToString(anz_brat_dm), " Gebratenes Fleisch gebraten");

		PrintScreen	(Fleischlieferung, -1, YPOS_XPGained, FONT_ScreenSmall, 2);

		EDDA_BRAT_DAY = Wld_GetDay ();
	};
};				 
