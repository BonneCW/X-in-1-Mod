...
instance DIA_PYRKA_BIGPROB(C_Info)
{
	npc = KDF_500_Pyrokar;
	nr = 125;
	condition = dia_pyrka_bigprob_condition;
	information = dia_pyrka_bigprob_info;
	important = FALSE;
	permanent = FALSE;
	description = "Die Söldner haben ein Problem.";
};


func int dia_pyrka_bigprob_condition()
{
	if (Npc_KnowsInfo (other, dia_lii_bigprob) && (Kapitel == 5) && (SCHUTZSCHICHT == FALSE))
	{
		return TRUE;
	};
};

func void dia_pyrka_bigprob_info()
{
	AI_Output (other, self, "Dia_Pyrka_Bigprob_00");	//Die Söldner auf Onars hof haben ein Problem.
	AI_Output (self, other, "Dia_Pyrka_Bigprob_01");	//Und warum kommst du zu mir?
	AI_Output (other, self, "Dia_Pyrka_Bigprob_02");	//Es handelt isch dabei um eine magisches Problem:Vor dem Hof befindet sich eine Gruppe Orks ein Riesenork und 3 Orkschamanen. Sie scheinen unverwundbar zu sein!
	AI_Output (self, other, "Dia_Pyrka_Bigprob_03");	//Unverwundbar, sagst du? Das muss ein komplexer Schutzzauber sein. Ich habe da eine Vermutung: Es müsste ein Kettenschutzzauber sein.
	AI_Output (self, other, "Dia_Pyrka_Bigprob_04");	//Etwas anderes könnten die Orkschamanen nicht vollbringen. Doch diese Kette hat ein Anfangsglied, dass verwundbar ist.
	AI_Output (other, self, "Dia_Pyrka_Bigprob_05");	//Verwundbar?
	AI_Output (self, other, "Dia_Pyrka_Bigprob_06");	//Ja und nein. So einfach ist es wieder auch nicht. So ein Kettenschutzzauber muss ständig angepasst werden, um völligen Schutz zu garantieren.
	AI_Output (self, other, "Dia_Pyrka_Bigprob_07");	//Dabei ist es ein erheblicher Unterschied, ob er tagsüber oder nachts ausgeführt wird. Das ist die Schwäche!
	AI_Output (self, other, "Dia_Pyrka_Bigprob_08");	//In der Dämmerung muss der Zauber neu angepasst werden und einer der Schamanen wird kurzzeitig verwundbar.
	AI_Output (self, other, "Dia_Pyrka_Bigprob_09");	//Ist erstmal einer tot bricht der Schutzauber komplett zusammen! Das musst du ausnutzen!
	B_LogEntry (BIG, "Pyrokar, hat mir Die Schwäche der unverwundbaren Orks erklärt: Während der Dämmerung wird einer kurzzeitig verwundbar. Wenn ich es schaffe ihn zu töten, dann bricht der komplette Schutzzauber zusammen und alle Orks werden sterblich. ");
};
