...
func void DIA_Bennet_Present_Info ()
{
	...
	|| (hero.guild == GIL_STRF)
	{
		...
		AI_Output 	(self ,other,"DIA_Bennet_Present_06_17"); //Jedenfalls habe ich mich entschieden dir meinen Mythril-Fokus zu überlassen.
		AI_Output 	(self ,other,"DIA_Bennet_Present_06_18"); //Du und deine Leute, ihr schürft das Zeug doch da oben, oder?
	
		B_GiveInvItems (self, hero, ItMi_MFokus,1);
		B_LogEntry (TOPIC_urizielb,"Bennet hat mir den Mythril-Fokus als Dank für seine Rettung überlassen.");	
...
//*********************************************************************
//	Kannst du auch Statuetten machen?
//*********************************************************************
instance DIA_Bennet_Create_Statue (C_Info)
{
  npc = SLD_809_Bennet;
  nr = 9;
  condition = DIA_Bennet_Create_Statue_Condition;
  information = DIA_Bennet_Create_Statue_Info;
  permanent = TRUE;
  important = false;
  description = "Kannst du auch Statuetten herstellen?";
};

func int DIA_Bennet_Create_Statue_Condition ()
{
  if ((Dark_Prüfung_Info == TRUE) && (other.guild == GIL_Bad) && !(SC_isDarkMage || SC_isDarkWarrior) && (MIS_RescueBennet == LOG_SUCCESS))
    {
      return true;
    };
  return false;
...
func void DIA_Bennet_Create_Statue_Info ()
{
  AI_Output(other, self, "Bennet_Create_Statue_01"); //Kannst du auch Statuetten herstellen?
  AI_Output(self, other, "Bennet_Create_Statue_02"); //Bist du so eitel geworden, dass du eine Statuette von dir brauchst? (lacht) Naja, meinetwegen.
  AI_Output(other, self, "Bennet_Create_Statue_03"); //Nein, das ist es nicht, ich möchte eine Statuette von Beliar haben!
  AI_Output(self, other, "Bennet_Create_Statue_04"); //(erstaund) Was, ist das dein Ernst? Mir soll's recht sein, voraus soll ich sie denn machen?
  Info_ClearChoices(DIA_Bennet_Create_Statue);
  if ((Npc_KnowsInfo(other, B5_9000_Mage_Ringe_Info) == TRUE) && (Npc_HasItems(other, ItMi_BeliarRing2) > 0))
    {
      Info_AddChoice(DIA_Bennet_Create_Statue, "Stelle mir eine Statuette aus diesen Ringen her (Beliars Wille)!", DIA_Bennet_Create_Statue_Ring);
    };
  Info_AddChoice(DIA_Bennet_Create_Statue, "Mach sie aus Gold!", DIA_Bennet_Create_Statue_Gold);
  Info_AddChoice(DIA_Bennet_Create_Statue, "Benutze einfach Stahl dafür!", DIA_Bennet_Create_Statue_Stahl);
};

func void DIA_Bennet_Create_Statue_Ring ()
{
  AI_Output(other, self, "Bennet_Create_Statue_05"); //Stelle mir eine Statuette aus diesen Ringen her!
  B_GiveInvItems(other, self, ItMi_BeliarRing2, 1);
  AI_Output(self, other, "Bennet_Create_Statue_06"); //Was zur Hölle ist das? Dieser Ring glüht ja! Mhh, es wäre durchaus möglich, aber dafür brauche ich mehr davon, so ungefähr zehn Stück!
  Info_ClearChoices(DIA_Bennet_Create_Statue);
  Info_AddChoice(DIA_Bennet_Create_Statue, "Hier hast du die Ringe!", DIA_Bennet_Create_Statue_Ring_Yes);
  Info_AddChoice(DIA_Bennet_Create_Statue, "Ich überlege es mir noch einmal.", DIA_Bennet_Create_Statue_Ring_No);
};

func void DIA_Bennet_Create_Statue_Ring_Yes ()
{
  AI_Output(other, self, "Bennet_Create_Statue_07"); //Hier hast du die Ringe!
  if (Npc_HasItems(other, ItMi_BeliarRing2) >= 9)
    {
      B_GiveInvItems(other, self, ItMi_BeliarRing2, 9);
      Npc_RemoveInvItems(self, ItMi_BeliarRing2, 10);
      CreateInvItems(self, ItMi_RingBeliarStatue, 1);
      AI_Output(self, other, "Bennet_Create_Statue_08"); //Ein seltsames Metall, es ist beinahe von allein in diese Form geflossen.
      B_GiveInvItems(self, other, ItMi_RingBeliarStatue, 1);
      AI_Output(self, other, "Bennet_Create_Statue_09"); //Weil du mich aus dem Knast geholt hast, kostet es dich nichts.
    }
  else
    {
      AI_Output(self, other, "Bennet_Create_Statue_10"); //Hörst du mir eigentlich zu? Du brauchst 10 Ringe davon!
      B_GiveInvItems(self, other, ItMi_BeliarRing2, 1);
    };
  Info_ClearChoices(DIA_Bennet_Create_Statue);
};

func void DIA_Bennet_Create_Statue_Ring_No ()
{
  AI_Output(other, self, "Bennet_Create_Statue_11"); //Ich überlege es mir noch einmal.
  AI_Output(self, other, "Bennet_Create_Statue_12"); //Wie du meinst, hier hast du deinen Ring zurück!
  B_GiveInvItems(self, other, ItMi_BeliarRing2, 1);
  Info_ClearChoices(DIA_Bennet_Create_Statue);
};

func void DIA_Bennet_Create_Statue_Gold ()
{
  AI_Output(other, self, "Bennet_Create_Statue_13"); //Mach sie aus Gold!
  AI_Output(self, other, "Bennet_Create_Statue_14"); //Das kostet dich 100 Gold! Und da verdiene ich nichts dran, das ist nur die Statuette!
  Info_ClearChoices(DIA_Bennet_Create_Statue);
  Info_AddChoice(DIA_Bennet_Create_Statue, "Hier hast du dein Gold!", DIA_Bennet_Create_Statue_Gold_Yes);
  Info_AddChoice(DIA_Bennet_Create_Statue, "Ich überlege es mir noch einmal.", DIA_Bennet_Create_Statue_No);
};

func void DIA_Bennet_Create_Statue_Gold_Yes ()
{
  AI_Output(other, self, "Bennet_Create_Statue_15"); //Hier hast du dein Gold!
  if (Npc_HasItems(other, ItMi_Gold) >= 100)
    {
      B_GiveInvItems(other, self, ItMi_Gold, 100);
      CreateInvItems(self, ItMi_GoldBeliarStatue, 1);
      AI_Output(self, other, "Bennet_Create_Statue_16"); //Ich hier hast du deine Statue, aber wenn du mich fragst, hat das Gold dadurch nur seinen Wert verloren.
      B_GiveInvItems(self, other, ItMi_GoldBeliarStatue, 1);
    }
  else
    {
      AI_Output(self, other, "Bennet_Create_Statue_17"); //Ich habe gesagt: Du brauchst 100 Goldstücke!
    };
  Info_ClearChoices(DIA_Bennet_Create_Statue);
};

func void DIA_Bennet_Create_Statue_No ()
{
  AI_Output(other, self, "Bennet_Create_Statue_18"); //Ich überlege es mir noch einmal.
  AI_Output(self, other, "Bennet_Create_Statue_19"); //Wie du meinst!
  Info_ClearChoices(DIA_Bennet_Create_Statue);
};

func void DIA_Bennet_Create_Statue_Stahl ()
{
  AI_Output(other, self, "Bennet_Create_Statue_20"); //Benutze einfach Stahl dafür!
  AI_Output(self, other, "Bennet_Create_Statue_21"); //Das kostet dich 50 Gold! Irgendwie muss ich ja den Stahl bezahlen! 
  Info_ClearChoices(DIA_Bennet_Create_Statue);
  Info_AddChoice(DIA_Bennet_Create_Statue, "Hier hast du dein Gold!", DIA_Bennet_Create_Statue_Stahl_Yes);
  Info_AddChoice(DIA_Bennet_Create_Statue, "Ich überlege es mir noch einmal.", DIA_Bennet_Create_Statue_No);
};

func void DIA_Bennet_Create_Statue_Stahl_Yes ()
{
  AI_Output(other, self, "Bennet_Create_Statue_22"); //Hier hast du dein Gold!
  if (Npc_HasItems(other, ItMi_Gold) >= 50)
    {
      B_GiveInvItems(other, self, ItMi_Gold, 50);
      CreateInvItems(self, ItMi_StahlBeliarStatue, 1);
      AI_Output(self, other, "Bennet_Create_Statue_23"); //Ich hier hast du deine Statue. Irgendwie hat sich der Stahl geweigert diese Form anzunehmen, aber ich habe es dennoch hinbekommen.
      B_GiveInvItems(self, other, ItMi_StahlBeliarStatue, 1);
    }
  else
    {
      AI_Output(self, other, "Bennet_Create_Statue_24"); //Du brauchst 50 Goldstücke! Ich bekomme den Stahl nicht geschenkt!
    };
  Info_ClearChoices(DIA_Bennet_Create_Statue);
};
