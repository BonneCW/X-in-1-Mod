...
INSTANCE DIA_Mil_305_Torwache_PassAsDarkMage (C_INFO)
{
	npc			= Mil_305_Torwache;
	nr			= 3;
	condition	= DIA_Mil_305_Torwache_PassAsDarkMage_Condition;
	information	= DIA_Mil_305_Torwache_PassAsDarkMage_Info;
	permanent 	= TRUE; 
	description	= "Du wagst es, dich mir in den Weg zu stellen ?!";
};                       

FUNC INT DIA_Mil_305_Torwache_PassAsDarkMage_Condition()
{	
...
FUNC VOID DIA_Mil_305_Torwache_PassAsDarkMage_Info()
{
	AI_Output (other, self,"DIA_Mil_305_Torwache_PassAsDarkMage_15_00"); //Du wagst es, dich mir in den Weg zu stellen?!
	AI_Output (self, other,"DIA_Mil_305_Torwache_PassAsDarkMage_03_01"); //Ähm ... Warum nicht?
	AI_Output (other, self,"DIA_Mil_305_Torwache_PassAsDarkMage_15_02"); //Der Letzte der das getan hat, bereut es noch heute.
	AI_Output (self, other,"DIA_Mil_305_Torwache_PassAsDarkMage_03_03"); //Nun gut, ich spüre die Macht die euch umgibt, tretet ein.
...
INSTANCE DIA_Mil_305_Torwache_PassAsgilde (C_INFO)
{
	npc			= Mil_305_Torwache;
	nr			= 3;
	condition	= DIA_Mil_305_Torwache_PassAsgilde_Condition;
	information	= DIA_Mil_305_Torwache_PassAsgilde_Info;
	permanent 	= TRUE; 
	description	= "Ich würde an deiner Stelle aus dem Weg gehen, Abschaum!";
};                       

FUNC INT DIA_Mil_305_Torwache_PassAsgilde_Condition()
{	
...
FUNC VOID DIA_Mil_305_Torwache_PassAsgilde_Info()
{
	AI_Output (other, self,"DIA_Mil_305_Torwache_PassAsgilde_15_00"); //Ich würde an deiner Stelle aus dem Weg gehen, Abschaum!
	AI_Output (self, other,"DIA_Mil_305_Torwache_PassAsgilde_03_01"); //Was du nennst einen Paladin Abschaum?!? Ich sollte dich...
	AI_Output (other, self,"DIA_Mil_305_Torwache_PassAsgilde_15_02"); //So redet man nicht zum 1. Offizier der Gilde. Bete dass du nicht mit aufgeschlitzter Kehle aufwachst!
	AI_Output (self, other,"DIA_Mil_305_Torwache_PassAsgilde_03_03"); //Is ja schon gut. Kannst reingehen... 
...
instance DIA_MIL_305_TORWACHE_PASSASVAM(C_Info)
{
	npc = Mil_305_Torwache;
	nr = 3;
	condition = dia_mil_305_torwache_passasvam_condition;
	information = dia_mil_305_torwache_passasvam_info;
	permanent = TRUE;
	description = "Ich habe hier einen Passierschein.";
};


func int dia_mil_305_torwache_passasvam_condition()
{
	if ((Mil_305_schonmalreingelassen == FALSE) && (B_GetGreatestPetzCrime (self) < CRIME_ATTACK) && (Npc_HasItems (other, itwr_passardoc) >= 1) && (ZETTEL05 == FALSE))
	{
		return TRUE;
	};
};

func void dia_mil_305_torwache_passasvam_info()
{
	AI_Output (other, self, "DIA_MIL_305_Torwache_PassAsVAM_15_00");	//Dieses Dokument gewährt mir den Zugang zum Oberen Viertel!
	B_UseFakeScroll ();
	AI_Output (self, other, "DIA_MIL_305_Torwache_PassAsVAM_12_01");	//Na und? Der klare Befehl Lord Hagens lautet: Nur Bürger und Truppen des Königs dürfen in das Obere Viertel!
	AI_Output (self, other, "DIA_MIL_305_Torwache_PassAsVAM_12_02");	//Du kannst wiederkommen, wenn du Bürger bist.
	AI_StopProcessInfos (self);
	ZETTEL05 = TRUE;
};
...
FUNC VOID DIA_Mil_305_Torwache_PERM_Info()
{
	...
	||		(other.guild == GIL_DJG)
	{
        ...
        else if (other.guild == GIL_STRF)
	||		(other.guild == GIL_BAU)
	{
		AI_Output (self, other,"DIA_Mil_305_Torwache_PERM_03_04"); //Ist ja schon gut...
	}
	else //GIL_None 
	{
