...
{
	if ( (other.guild == GIL_MIL) || (other.guild == GIL_PAL) || (other.guild == GIL_NOV) || (other.guild == GIL_KDF) || (other.guild == GIL_Bad))
	&& (Lee_IsOnBoard == FALSE)
...
FUNC VOID DIA_Lee_KeinSld_Info()
{
	...
	if (other.guild == GIL_Bad)
	  {
	    AI_Output(self, other, "DIA_Lee_KeinSld_04_06"); //Du hast dich den dunklen Mächten angeschlossen? Wenn das mal gut geht.
	  };
...
func void DIA_Lee_GetShip_Info ()
{
	...
	Log_SetTopicStatus(TOPIC_Ship, LOG_RUNNING);                                                                                                                                                                                                                                                 
		...
		else if ((hero.guild == GIL_SLD)||(hero.guild == GIL_DJG) || (hero.guild == GIL_OUT) || (hero.guild == GIL_BAU))
		{
...
instance DIA_LII_BIGPROB(C_Info)
{
	npc = SLD_800_Lee;
	nr = 125;
	condition = dia_lii_bigprob_condition;
	information = dia_lii_bigprob_info;
	important = TRUE;
	permanent = FALSE;
};


func int dia_lii_bigprob_condition()
{
	if (Kapitel == 5)
	{
		return TRUE;
...
func void dia_lii_bigprob_info()
{
	AI_Output (self, other, "Dia_Lii_Bigprob_00");	//Wir haben ein Problem, ein ziemlich großes Problem!
	AI_Output (other, self, "Dia_Lii_Bigprob_01");	//Was meinst du?
	AI_Output (self, other, "Dia_Lii_Bigprob_02");	//Hast du denn nicht den Riesenork vor dem Hof gesehen? Das Ding ist mindesten 3 Meter Groß! Und 3 Orkschamanen sind auch noch bei ihm.
	AI_Output (other, self, "Dia_Lii_Bigprob_03");	//Sowas ist doch für Söldner kein Problem?!
	AI_Output (self, other, "Dia_Lii_Bigprob_04");	//Oh, doch! Wir haben die Biester angegriffen, aber sie scheinen alle unverwundbar zu sein!
	AI_Output (other, self, "Dia_Lii_Bigprob_05");	//Unverwundbar?
	AI_Output (self, other, "Dia_Lii_Bigprob_06");	//Ja, unverwundbar! Wir konnten sie mit unseren Schwertern nicht einmal ankratzen! Das muss Magie sein!
	AI_Output (other, self, "Dia_Lii_Bigprob_07");	//Habt ihr nicht die Feuermagier um Rat gefragt?
	AI_Output (self, other, "Dia_Lii_Bigprob_08");	//Darum wollte ich dich ja bitten! Geh zu ihnen und finde heraus, wie man diese Biester kalt stellt!
	AI_StopProcessInfos (self);
	Log_CreateTopic (BIG, LOG_MISSION);
	Log_SetTopicStatus (BIG, LOG_Running);
	B_LogEntry (BIG, "Vor dem Hof befindet sich eine unbesiegbare Gruppe Orks, darunter ein Riesenork. Ich soll die Feuermagier um Rat fragen.");
};


instance DIA_LII_GRETJOB(C_Info)
{
	npc = SLD_800_Lee;
	nr = 126;
	condition = dia_lii_gretjob_condition;
	information = dia_lii_gretjob_info;
	important = TRUE;
	permanent = FALSE;
};


func int dia_lii_gretjob_condition()
{
	if (Npc_KnowsInfo (other, dia_lii_bigprob) && Npc_IsDead (orcshaman_hof1) && Npc_IsDead (orcshaman_hof2) && Npc_IsDead (orcshaman_hof3) && Npc_IsDead (orcwarrior_giant) && (Kapitel == 5))
	{
		return TRUE;
	};
};

func void dia_lii_gretjob_info()
{
	AI_Output (other, self, "Dia_Lii_gretjob_00");	//Die Orks sind tot!
	AI_Output (self, other, "Dia_Lii_gretjob_01");	//Tot? Du hast es geschafft sie zu töten?!
	AI_Output (self, other, "Dia_Lii_gretjob_02");	//Also manchmal glaube ich ernsthaft, du wärst kein normaler Mensch (lacht)!
	AI_Output (self, other, "Dia_Lii_gretjob_03");	//Ich schulde dir großen Dank! Du hast den Hof gerettet!
	B_GivePlayerXP (2000);
	Log_SetTopicStatus (BIG, LOG_SUCCESS);
};
