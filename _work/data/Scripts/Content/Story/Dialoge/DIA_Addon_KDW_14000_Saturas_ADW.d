...
func void DIA_Addon_Saturas_PermENDE_ADDON_Info ()
{
	...
	|| (hero.guild == GIL_BAD))
	{
		Info_AddChoice (DIA_Addon_Saturas_PermENDE_ADDON, "(Ich will Wassermagier werden!)", dia_addon_saturas_adwstart_kdw);
		Info_AddChoice (DIA_Addon_Saturas_PermENDE_ADDON, Dialog_Back, dia_addon_saturas_permende_addon_back);
	};*/
};

func void dia_addon_saturas_permende_addon_back()
{
	Info_ClearChoices (DIA_Addon_Saturas_PermENDE_ADDON);
};

func void dia_addon_saturas_adwstart_kdw()
{
	Info_ClearChoices (DIA_Addon_Saturas_PermENDE_ADDON);
	Info_AddChoice (DIA_Addon_Saturas_PermENDE_ADDON, "Vielleicht ein anderes mal..", dia_addon_saturas_adwstart_kdw_back);
	Info_AddChoice (DIA_Addon_Saturas_PermENDE_ADDON, "Eid des Wassers ablegen !!", dia_addon_saturas_adwstart_kdw_join);
};

func void dia_addon_saturas_adwstart_kdw_back()
{
	Info_ClearChoices (DIA_Addon_Saturas_PermENDE_ADDON);
};

func void dia_addon_saturas_adwstart_kdw_join()
{
	Info_ClearChoices (DIA_Addon_Saturas_PermENDE_ADDON);
	CreateInvItems (other, itar_kdw_l_addon, 1);
	//AI_EquipArmor (other, itar_kdw_l_addon);
	if (RitualInnosEyeRuns == LOG_SUCCESS)
	{
	...
	if (SLD_Aufnahme != LOG_SUCCESS)
	{
		SLD_Aufnahme = LOG_OBSOLETE;
	};
	if (MIL_Aufnahme != LOG_SUCCESS)
	{
		MIL_Aufnahme = LOG_OBSOLETE;
	};
	Wld_ExchangeGuildAttitudes ("GIL_ATTITUDES");
	CreateInvItems (Cronos_ADW, itbe_addon_kdw_01, 1);
	...
	B_LogEntry (TOPIC_BECOMEKDW, "Durch Ravens Vernichtung konnte ich letztendlich das Vertrauen von Saturas zurückgewinnen, und er hat mich in den Kreis der Wassermagier aufgenommen.");
