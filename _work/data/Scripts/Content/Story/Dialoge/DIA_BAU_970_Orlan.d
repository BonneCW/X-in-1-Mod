...
instance DIA_ORLAN_RITUALMESSER(C_Info)
{
	npc = BAU_970_Orlan;
	nr = 700;
	condition = dia_orlan_ritualmesser_condition;
	information = dia_orlan_ritualmesser_info;
	important = FALSE;
	permanent = FALSE;
	description = "Ich suche ein spezielles Messer.";
};


func int dia_orlan_ritualmesser_condition()
{
	if (Npc_KnowsInfo (other, dia_dimitri_ritual))
	{
		return TRUE;
	};
};

func void dia_orlan_ritualmesser_info()
{
	AI_Output (other, self, "Dia_Orlan_ritualmesser_00");	//Ich suche ein spezielles Messer. Dimitri sagt, er hätte es dir verkauft.
	AI_Output (self, other, "Dia_Orlan_ritualmesser_01");	//Was für ein Messer? Achso, jetzt erinnere ich mich wieder.
	AI_Output (self, other, "Dia_Orlan_ritualmesser_02");	//Dimitri hat mir mal ein ziemlich schönes und wertvolles Messer verkauft.
	AI_Output (other, self, "Dia_Orlan_ritualmesser_03");	//Hast du es noch?
	AI_Output (self, other, "Dia_Orlan_ritualmesser_04");	//Nein, ich habs weiterverkauft. Ein Jäger hat sich dafür interessiert.
	AI_Output (other, self, "Dia_Orlan_ritualmesser_05");	//Wie hieß er?
	AI_Output (self, other, "Dia_Orlan_ritualmesser_06");	//Keine Ahnung! Es ist zu lange her. Er war nur hier um sich ein neues Messer zu besorgen.
	AI_Output (self, other, "Dia_Orlan_ritualmesser_07");	//Ich glaub sein Name fing mit "N" an. An mehr kann ich mich nicht erinnern.
	AI_Output (other, self, "Dia_Orlan_ritualmesser_08");	//Danke für die Hilfe.
	B_LogEntry (DAS_BLUTRITUAL, "Orlan hat das Ritualmesser an einen Jäger verkauft, dessen Name mit 'N' anfängt. Ich muss diesen finden.");
	B_GivePlayerXP (100);
};	
