...
instance DIA_LOTHI_GERBRANDSBUDE(C_Info)
{
	npc = PAL_203_Lothar;
	nr = 546;
	condition = dia_lothi_gerbrandsbude_condition;
	information = dia_lothi_gerbrandsbude_info;
	important = TRUE;
	permanent = FALSE;
};


func int dia_lothi_gerbrandsbude_condition()
{
	if ((Mil_305_schonmalreingelassen == TRUE) && Npc_KnowsInfo (other, DIA_Lothar_HelloAgain))
	{
		return TRUE;
	};
};

func void dia_lothi_gerbrandsbude_info()
{
	AI_Output (self, other, "Dia_Lothi_Gerbrandsbude_00");	//Ach, übrigens: In Gerbrands Haus ist vor Kurzem jemand eingebrochen.
	AI_Output (self, other, "Dia_Lothi_Gerbrandsbude_01");	//Der Dieb hat ihm einen Beutel voller Gold gestohlen.
	AI_Output (self, other, "Dia_Lothi_Gerbrandsbude_02");	//Nicht, dass das für einen reichen Mann, wie ihn viel ist, aber so ein Verbrechen, darf nicht ungestraft bleiben.
	AI_Output (self, other, "Dia_Lothi_Gerbrandsbude_03");	//Sieh dich am Besten bei Gerbrands Haus um, vielleicht findst du dort eine Spur!
	Log_CreateTopic (GERBRANDSBUDE, LOG_MISSION);
	Log_SetTopicStatus (GERBRANDSBUDE, LOG_Running);
	B_LogEntry (GERBRANDSBUDE, "In Gerbrands Heus wurde eingebrochen. Ich soll mich dort nach Spuren umsehen, um den Täter zu finden.");
	AI_StopProcessInfos (self);
};


instance DIA_LOTHI_GERBRANDSBUDE_END(C_Info)
{
	npc = PAL_203_Lothar;
	nr = 547;
	condition = dia_lothi_gerbrandsbude_end_condition;
	information = dia_lothi_gerbrandsbude_end_info;
	important = FALSE;
	permanent = FALSE;
	description = "Ich habe den Dieb gefunden";
};


func int dia_lothi_gerbrandsbude_end_condition()
{
	if ((READ_DROHDING == TRUE) && Npc_KnowsInfo (other, dia_lothi_gerbrandsbude) && (Npc_HasItems (other, itwr_drohding) >= 1))
	{
		return TRUE;
	};
};

func void dia_lothi_gerbrandsbude_end_info()
{
	AI_Output (other, self, "Dia_Lothi_Gerbrandsbude_end_00");	//Ich habe den Dieb gefunden. Er ist tot.
	AI_Output (self, other, "Dia_Lothi_Gerbrandsbude_end_01");	//Was?
	AI_Output (other, self, "Dia_Lothi_Gerbrandsbude_end_02");	//Ich fand seine Leiche. Hier, diesen Zettel hatte er bei sich.
	B_UseFakeScroll ();
	AI_Output (self, other, "Dia_Lothi_Gerbrandsbude_end_03");	//Seltsam... Aber nun gut. Er hat seine Strafe erhalten und damit sollte der Fall geklärt sein.
	Log_SetTopicStatus (GERBRANDSBUDE, LOG_SUCCESS);
	B_GivePlayerXP (250);
	AI_StopProcessInfos (self);
};
