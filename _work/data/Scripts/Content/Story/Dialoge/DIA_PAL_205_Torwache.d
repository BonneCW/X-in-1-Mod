...
INSTANCE DIA_PAL_205_Torwache_PassAsDarkMage (C_INFO)
{
	npc			= PAL_205_Torwache;
	nr			= 3;
	condition	= DIA_PAL_205_Torwache_PassAsDarkMage_Condition;
	information	= DIA_PAL_205_Torwache_PassAsDarkMage_Info;
	permanent 	= TRUE; 
	description	= "Ich habe hier ein göttliches Artefakt, dass sollte Lord Hagen interessieren.";
};                       

FUNC INT DIA_PAL_205_Torwache_PassAsDarkMage_Condition()
{	
  if (Npc_GetTrueGuild(other) == GIL_Bad)
    && (PAL_205_schonmalreingelassen == FALSE)
      && (B_GetGreatestPetzCrime(self) < CRIME_ATTACK)
      && (Npc_HasItems(other, Holy_Hammer_Mis) == 1)
	{
		return TRUE;
	};
};

FUNC VOID DIA_PAL_205_Torwache_PassAsDarkMage_Info()
{
	AI_Output (other, self,"DIA_PAL_205_Torwache_PassAsDarkMage_15_00"); //Ich habe hier ein göttliches Artefakt, dass sollte Lord Hagen interessieren.
	AI_Output (self, other,"DIA_PAL_205_Torwache_PassAsDarkMage_12_01"); //Was, wie soll ein Diener der Schatten an ein göttliches Artefakt kommen?
	AI_Output (other, self,"DIA_PAL_205_Torwache_PassAsDarkMage_12_02"); //Wie ist egal, Tatsache ist, das ich rangekommen bin.
	AI_Output (self, other,"DIA_PAL_205_Torwache_PassAsDarkMage_12_03"); //Nun gut, es soll dir gestattet sein, das Rathaus zu betreten.
...
INSTANCE DIA_PAL_205_Torwache_PassAsgilde (C_INFO)
{
	npc			= PAL_205_Torwache;
	nr			= 3;
	condition	= DIA_PAL_205_Torwache_PassAsgilde_Condition;
	information	= DIA_PAL_205_Torwache_PassAsgilde_Info;
	permanent 	= TRUE; 
	description	= "Ich bin der 1. Offizier der Gilde und will zu Lord Hagen";
};                       

FUNC INT DIA_PAL_205_Torwache_PassAsgilde_Condition()
{	
	if (Npc_GetTrueGuild(other) == GIL_STRF)
	&& (PAL_205_schonmalreingelassen == FALSE)
	&& (B_GetGreatestPetzCrime(self) < CRIME_ATTACK)
	{
		return TRUE;
...
FUNC VOID DIA_PAL_205_Torwache_PassAsgilde_Info()
{
	AI_Output (other, self,"DIA_PAL_205_Torwache_PassAsgilde_15_00"); //Ich bin der 1. Offizier der Gilde und will zu Lord Hagen
	AI_Output (self, other,"DIA_PAL_205_Torwache_PassAsgilde_12_01"); //Die Gilde oben am Leuchtturm? ....Ihr versorgt die Stadt mit Fleisch und Fellen. 
	AI_Output (self, other,"DIA_PAL_205_Torwache_PassAsgilde_12_02"); //In Ordung, du kannst rein.
	self.aivar[AIV_PASSGATE] = TRUE;
	PAL_205_schonmalreingelassen = TRUE;
	AI_StopProcessInfos	(self);
};
