...
{	
		...
		if ((hero.guild == GIL_KDF) || (hero.guild == GIL_Bad))
		{
		...
		{
			AI_Output (self ,other,"DIA_Garond_Hello_10_06"); //Ich frage mich, warum du hier bist. Was willst du hier?
		};
};

INSTANCE DIA_Garond_Nordmann (C_INFO)
{
	npc		= PAL_250_Garond;
	nr		= 4;
	condition	= DIA_Garond_Nordmann_Condition;
	information	= DIA_Garond_Nordmann_Info;
	IMPORTANT 	= 0;
	permanent	= 0;
	description	= "Was weißt du über den Nordmarer, der vor kurzem in der Burg war?";
};                       

FUNC INT DIA_Garond_Nordmann_Condition()
{
	if Npc_KnowsInfo (other, DIA_Tandor_Nordmann)
	{
		return 1;
	};	
};

FUNC VOID DIA_Garond_Nordmann_Info()
{	
	AI_Output (other ,self,"DIA_Garond_Nordmann_15_00"); //Was weißt du über den Nordmarer, der vor kurzem in der Burg war?
	AI_Output (self ,other,"DIA_Garond_Nordmann_10_01"); //Nicht viel, er hat nur mit Oric gesprochen.
	AI_Output (self ,other,"DIA_Garond_Nordmann_10_02"); //Aber er schien mir ein starker Krieger zu sein, der schon viele Kämpfe bestritten hat.
	AI_Output (self ,other,"DIA_Garond_Nordmann_10_03"); //Ich hätte ihn wirklich gern hier behalten, jemanden wie ihn hätten wir im Falle eines erneuten Angriffs gut gebrauchen können.
	AI_Output (other ,self,"DIA_Garond_Nordmann_15_04"); //Soll das heißen, dass er nicht mehr hier ist?
	AI_Output (self ,other,"DIA_Garond_Nordmann_10_05"); //Exakt.
	AI_Output (other ,self,"DIA_Garond_Nordmann_15_06"); //Weißt du, wohin er wollte?
	AI_Output (self ,other,"DIA_Garond_Nordmann_10_07"); //Wie gesagt, ich habe nicht mit ihm gesprochen. Aber Oric weiß sicher mehr darüber.

	B_LogEntry	(TOPIC_DGDH_NORDMANN, "Lord Hagen konnte mir auch nichts genaues über den Nordmarer sagen. Nur, dass er mit Oric gesprochen hat und nun nicht mehr in der Burg ist.");
};
...
{		
	...
	Npc_ExchangeRoutine (B5_90020_Helmchen_OW, "Wait_DT");
...
INSTANCE DIA_Garond_rl (C_INFO)
{
	npc			= PAL_250_Garond;
	nr			= 4;
	condition	= DIA_Garond_rl_Condition;
	information	= DIA_Garond_rl_Info;
	permanent	= FALSE;
	description = "Ich komme gerade von den Schürfstellen.";
};                       

FUNC INT DIA_Garond_rl_Condition()
{
...
FUNC VOID DIA_Garond_rl_Info()
{		
	AI_Output (other,self ,"DIA_Garond_rl_15_00"); //Ich komme gerade von den Schürfstellen.
	AI_Output (self ,other,"DIA_Garond_rl_10_01"); //Na dann spucks aus! Wie viele Kisten Erz haben wir?
	AI_Output (other,self ,"DIA_Garond_rl_10_02"); //Nicht eine!
	AI_Output (self ,other,"DIA_Garond_rl_10_03"); //Was? 
	AI_Output (other,self ,"DIA_Garond_rl_10_04"); //Alle Schürfstellen wurden von Orks verwüstet.
	AI_Output (self ,other,"DIA_Garond_rl_10_05"); //Das darf nicht wahr sein.... 
	AI_Output (self ,other,"DIA_Garond_rl_10_06"); //Aber ich kann dir das Schreiben noch nicht aushändigen. Ich muss noch warten, ob sich ein Überlebender findet.
	B_KillNpc(PAL_281_Fajeth);
	B_KillNpc(PAL_217_Marcos);

	B_LogEntry (TOPIC_ScoutMine,"Ich habe Garond belogen, so wie Reto es gesagt hat.");

	B_LogEntry (TOPIC_ERZV,"Ich habe Garond belogen, so wie Reto es gesagt hat. Garond will jetzt aber noch warten, ob sich Überlebende finden. Ich sollte mal mit Oviedo reden.");

	MIS_ScoutMine = LOG_SUCCESS;
	Log_SetTopicStatus(TOPIC_ScoutMine, LOG_SUCCESS);
	Log_SetTopicStatus(TOPIC_ERZV, LOG_SUCCESS);
	B_CheckLog();
};

INSTANCE DIA_Garond_Successx (C_INFO)
{
	npc			= PAL_250_Garond;
	nr			= 4;
	condition	= DIA_Garond_Successx_Condition;
	information	= DIA_Garond_Successx_Info;
	permanent	= FALSE;
	description = "Was ist mit dem Schreiben für Lord Hagen?";
};                       

FUNC INT DIA_Garond_Successx_Condition()
{
	if (Npc_KnowsInfo (other,DIA_Vlk_6020_Oviedo_urizielda))
	&& (Kapitel == 2)
...
FUNC VOID DIA_Garond_Successx_Info()
{		
	AI_Output (other,self ,"DIA_Garond_Successx_15_00"); //Was ist mit dem Schreiben für Lord Hagen?
	AI_Output (self ,other,"DIA_Garond_Successx_10_01"); //Du wirst dein Schreiben bekommen. Lord Hagen MUSS davon erfahren. Dieses Tal ist verflucht - das Böse ist hier.
...
//Den Statthalter-Brief von Garond unterzeichnen lassen
instance Garond_Statthalter (C_Info)
{
  npc = Pal_250_Garond;
  nr = 13;
  condition = Garond_Statthalter_Condition;
  information = Garond_Statthalter_Info;
  description = "Mud sagt, du sollst diesen Brief unterschreiben.";
};

func int Garond_Statthalter_Condition ()
{
  if (Npc_HasItems(other, ItWr_Stadthalter) > 0)
    {
      return true;
    };
  return false;
...
func void Garond_Statthalter_Info ()
{
  AI_Output(other, self, "Garond_Statthalter_01"); //Mud sagt, du sollst diesen Brief unterschreiben.
  AI_Output(self, other, "Garond_Statthalter_02"); //Zeig her!
  B_GiveInvItems(other, self, ItWr_Stadthalter, 1);
  B_UseFakeScroll();
  AI_Output(self, other, "Garond_Statthalter_03"); //Da hat dir der Erzbaron ja eine große Aufgabe erteilt, sollte der alte Statthalter Probleme machen, dann sprich mit Lord Hagen.
  AI_Output(self, other, "Garond_Statthalter_04"); //Hier hast du dein Schreiben zurück.
  CreateInvItems(self, ItWr_Stadthalter2, 1);
  B_GiveInvItems(self, other, ItWr_Stadthalter2, 1);
  B_LogEntry(Statthalter, "Das Schreiben sollte mir den alten Statthalter vom Hals schaffen.");
  B_GivePlayerXP(XP_Ambient);
};

//Verhandlungen mit dem orkischen Anführer
instance Garond_Verhandlungen (C_Info)
{
  npc = Pal_250_Garond;
  nr = 14;
  condition = Garond_Verhandlungen_Condition;
  information = Garond_Verhandlungen_Info;
  description = "Weißt du etwas über einen orkischen Anführer?";
};

func int Garond_Verhandlungen_Condition ()
{
  return Npc_KnowsInfo(other, Bad_LAP_Schiff);
};

func void Garond_Verhandlungen_Info ()
{
  AI_Output(other, self, "Garond_Verhandlungen_01"); //Weißt du etwas über einen orkischen Anführer?
  AI_Output(self, other, "Garond_Verhandlungen_02"); //Wie kommst du denn darauf?
  if (Npc_KnowsInfo(other, Mud_Verhandlungen))
    {
      AI_Output(other, self, "Garond_Verhandlungen_03"); //Mud meinte du hättest danach gefragt.
    }
  else
    {
      AI_Output(other, self, "Garond_Verhandlungen_04"); //Reine Vermutung ...
    };
  AI_Output(self, other, "Garond_Verhandlungen_05"); //Ja, mir ist tatsächlich ein Anführer aufgefallen, er treibt sich manchmal am Haupttor rum und gibt den Orks neue Befehle.
  AI_Output(self, other, "Garond_Verhandlungen_06"); //Ich würde ihn am Liebsten erschießen lassen, aber er ist sehr vorsichtig.
  Wld_InsertNpc(Orc_9011_Nur_Shak, "OC_PATH_01");
  B_LogEntry(Verhandlungen, "Garond meint, dass ein orkischer Anführer sich in der Nähe des Haupttores aufhält.");
  B_GivePlayerXP(XP_Ambient);
};