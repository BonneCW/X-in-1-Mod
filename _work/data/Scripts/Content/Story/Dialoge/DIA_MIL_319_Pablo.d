...
instance DIA_PABLO_WANZEN(C_Info)
{
	npc = MIL_319_Pablo;
	nr = 15;
	condition = dia_pablo_wanzen_condition;
	information = dia_pablo_wanzen_info;
	permanent = FALSE;
	important = TRUE;
};


func int dia_pablo_wanzen_condition()
{
	if ((other.guild == GIL_NONE) && Npc_KnowsInfo (other, DIA_Pablo_WANTED))
	{
		return TRUE;
	};
};

func void dia_pablo_wanzen_info()
{
	AI_Output (self, other, "DIA_Pablo_WANZEN_12_10");	//Ach ja! Du bist neu hier und solltest du dich nützlich machen.
	AI_Output (self, other, "DIA_Pablo_Wanzen_12_12");	//Sonst fängst du vor Langeweile noch mit Diebstahl oder Schlimmerem an.
	AI_Output (self, other, "DIA_Pablo_WANZEN_12_13");	//Wir haben ein Wanzenproblem. Die Biester wimmeln hier so nur in der Unterstadt.
	AI_Output (self, other, "DIA_Pablo_WANZEN_12_14");	//(grinst)Die Viecher finden im Hafenviertel anscheinend nicht genug zu essen.
	AI_Output (other, self, "DIA_Pablo_Wanzen_12_15");	//Und was soll ich machen? Die Viecher jagen?
	AI_Output (self, other, "DIA_Pablo_WAnzen_12_16");	//Genau!
	Info_ClearChoices (dia_pablo_wanzen);
	Info_AddChoice (dia_pablo_wanzen, "Willst du mich verarschen?", dia_pablo_wanzen_no);
	Info_AddChoice (dia_pablo_wanzen, "Na gut, wenn ich mich nützlich machen kann.", dia_pablo_wanzen_yes);
};

func void dia_pablo_wanzen_yes()
{
	AI_Output (other, self, "DIA_Pablo_Wanzen_12_20");	//Na gut, wenn ich mich nützlich machen kann.
	AI_Output (self, other, "DIA_Pablo_WANZEN_12_21");	//Töte mindestens 6 Biester. Am besten du suchst in der Nähe von Orten, wo es was zu Essen für sie gibt.
	Log_CreateTopic (PABLO_WANZEN, LOG_MISSION);
	Log_SetTopicStatus (PABLO_WANZEN, LOG_Running);
	B_LogEntry (PABLO_WANZEN, "Ich soll für Pablo Fleischwanzen in der Unterstadt jagen. 6 Viecher sollten genügen.");
	AI_StopProcessInfos (self);
	WANZENOKAY = TRUE;
};

func void dia_pablo_wanzen_no()
{
	AI_Output (other, self, "DIA_Pablo_Wanzen_12_23");	//Willst du mich verarschen? Ich bin hier nicht der Kammerjäger!
	AI_Output (self, other, "DIA_Pablo_WANZEN_12_24");	//Dann verpiss dich!
	AI_StopProcessInfos (self);
};


instance DIA_PABLO_WANZEN_TOT(C_Info)
{
	npc = MIL_319_Pablo;
	nr = 16;
	condition = dia_pablo_wanzen_tot_condition;
	information = dia_pablo_wanzen_tot_info;
	permanent = FALSE;
	description = "Die Wanzen sind tot";
};


func int dia_pablo_wanzen_tot_condition()
{
	if ((WANZENOKAY == TRUE) && (MEATBUGHUNTER >= 6))
	{
		return TRUE;
	};
};

func void dia_pablo_wanzen_tot_info()
{
	AI_Output (other, self, "DIA_Pablo_WAn_15_08");	//Die Wanzen sind tot.
	AI_Output (self, other, "DIA_Pablo_wan_12_15");	//Gute Arbeit! Du bist ja doch zu was zu gebrauchen.
	AI_Output (self, other, "DIA_Pablo_wan_12_16");	//Hier nimm das Gold. Du hast es dir verdient.
	CreateInvItems (self, ItMi_Gold, 25);
	B_GiveInvItems (self, other, ItMi_Gold, 25);
	Log_SetTopicStatus (PABLO_WANZEN, LOG_SUCCESS);
	B_GivePlayerXP (100);
	AI_StopProcessInfos (self);
};
