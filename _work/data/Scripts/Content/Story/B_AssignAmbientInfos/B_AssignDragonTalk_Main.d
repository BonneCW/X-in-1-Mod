const string Dark_Waffe = "Die Dunkle Klinge";
// *************************************************************************
// 									Drachenfrage 1
// *************************************************************************
...
//*****************************
// Schwertweihe Dunkler Krieger
//*****************************

instance DIA_DragonTalk_Dark_1 (C_Info)
{
  nr = 1;
  condition = DIA_DragonTalk_Dark_1_Condition;
  information = DIA_DragonTalk_Dark_1_Info;
  description = "Beliar selbst hat mich entsandt dich zu töten.";
};

func int DIA_DragonTalk_Dark_1_Condition ()
{
  if ((MIS_KilledDragons == 0) && (hero.guild == GIL_Bad) && SC_isDarkWarrior)
    {
      return true;
    };
  return false;
...
func void DIA_DragonTalk_Dark_1_Info ()
{
  AI_Output(other, self, "DIA_DragonTalk_Dark_1_01"); //Beliar selbst hat mich entsandt dich zu töten.
  AI_Output(other, self, "DIA_DragonTalk_Dark_1_02"); //Durch seine Waffe wirst du fallen.
  AI_Output(self, other, "DIA_DragonTalk_Dark_1_03"); //Du armer Wicht, es würde schon genügen, deine Waffe mit dem Knochen eines Goblins zu verbinden, um sie gefährlicher zu machen.
  AI_Output(self, other, "DIA_DragonTalk_Dark_1_04"); //Und damit willst du mich töten? Mach dich doch nicht lächerlich!
  Log_CreateTopic(Dark_Waffe, LOG_NOTE);
  B_LogEntry(Dark_Waffe, "An einem Runentisch kann man die Dunkle Waffe mit einem Goblinknochen verbinden.");
};

instance DIA_DragonTalk_Dark_2 (C_Info)
{
  nr = 1;
  condition = DIA_DragonTalk_Dark_2_Condition;
  information = DIA_DragonTalk_Dark_2_Info;
  description = "Mit Beliars Hilfe werde ich dein Ende sein.";
};

func int DIA_DragonTalk_Dark_2_Condition ()
{
  if ((MIS_KilledDragons == 1) && (hero.guild == GIL_Bad) && SC_isDarkWarrior)
    {
      return true;
    };
  return false;
...
func void DIA_DragonTalk_Dark_2_Info ()
{
  AI_Output(other, self, "DIA_DragonTalk_Dark_2_01"); //Mit Beliars Hilfe werde ich dein Ende sein!
  AI_Output(self, other, "DIA_DragonTalk_Dark_2_02"); //Mein Ende? Vielleicht wenn du seine Kraft mit dem Fell eines Wolfes gesteigert hättest.
  AI_Output(self, other, "DIA_DragonTalk_Dark_2_03"); //Aber so? ICH werde DEIN Ende sein.
  B_LogEntry(Dark_Waffe, "An einem Runentisch kann man die Dunkle Waffe mit einem Wolfsfell verbinden.");
};

instance DIA_DragonTalk_Dark_3 (C_Info)
{
  nr = 1;
  condition = DIA_DragonTalk_Dark_3_Condition;
  information = DIA_DragonTalk_Dark_3_Info;
  description = "Deine Tage sind gezählt, Beliars Wille wird erfüllt.";
};

func int DIA_DragonTalk_Dark_3_Condition ()
{
  if ((MIS_KilledDragons == 2) && (hero.guild == GIL_Bad) && SC_isDarkWarrior)
    {
      return true;
    };
  return false;
...
func void DIA_DragonTalk_Dark_3_Info ()
{
  AI_Output(other, self, "DIA_DragonTalk_Dark_3_01"); //Deine Tage sind gezählt, Beliars Wille wird erfüllt.
  AI_Output(self, other, "DIA_DragonTalk_Dark_3_02"); //Und du glaubst, mich töten zu können? Verbinde deine Waffe mit dem Knochen eines Skeletts, um eine wirklich mächtige Waffe zu erhalten.
  AI_Output(self, other, "DIA_DragonTalk_Dark_3_03"); //Doch diese Zeit wird dir nicht mehr bleiben.
  B_LogEntry(Dark_Waffe, "An einem Runentisch kann man die Dunkle Waffe mit einem Skelettknochen verbinden.");
};

instance DIA_DragonTalk_Dark_4 (C_Info)
{
  nr = 1;
  condition = DIA_DragonTalk_Dark_4_Condition;
  information = DIA_DragonTalk_Dark_4_Info;
  description = "Glaubst auch du nicht an die Stärke meiner Waffe?";
};

func int DIA_DragonTalk_Dark_4_Condition ()
{
  if ((MIS_KilledDragons == 3) && (hero.guild == GIL_Bad) && SC_isDarkWarrior)
    {
      return true;
    };
  return false;
...
func void DIA_DragonTalk_Dark_4_Info ()
{
  AI_Output(other, self, "DIA_DragonTalk_Dark_4_01"); //Glaubst auch du nicht an die Stärke meiner Waffe?
  AI_Output(self, other, "DIA_DragonTalk_Dark_4_02"); //Ein schönes Stück, dass muss ich zugeben. Hättest du sie mit dem Herzen eines Golems gekreuzt, so wäre sie ein würdiges Stück für meinen Hort.
  AI_Output(self, other, "DIA_DragonTalk_Dark_4_03"); //So wird sie mit dir im Boden verfaulen.
  B_LogEntry(Dark_Waffe, "An einem Runentisch kann man die Dunkle Waffe mit einem (Stein-)Golemherz verbinden.");
};
...
FUNC VOID B_AssignDragonTalk_Main (var c_NPC slf)
{
	...
	DIA_DragonTalk_Dark_1.npc = Hlp_GetInstanceID(slf);
	DIA_DragonTalk_Dark_2.npc = Hlp_GetInstanceID(slf);
	DIA_DragonTalk_Dark_3.npc = Hlp_GetInstanceID(slf);
	DIA_DragonTalk_Dark_4.npc = Hlp_GetInstanceID(slf);
};
