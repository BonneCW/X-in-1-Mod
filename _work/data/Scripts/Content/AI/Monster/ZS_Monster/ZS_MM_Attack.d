...
{			
	...
	if (self.guild == GIL_SUMMONED_WOLF) && (F_DEF==TRUE)
	{
	self.FIGHT_tactic = FAI_DEFENCE;
	
	};
	if (self.guild == GIL_SUMMONED_WOLF) && (F_aggro==TRUE)
	{
	self.FIGHT_tactic = FAI_aggro;
	
	};
	if (self.guild == GIL_SUMMONED_WOLF) && (F_aggro==FALSE)&& (F_DEF==FALSE)
	{
	self.FIGHT_tactic = FAI_WOLF;
...
func int ZS_MM_Attack_Loop ()
{
	...
	// ------ Summon Time ------
	if (Npc_HasItems(hero, ItWr_CreaturePatch) == 0)
	{
		if (self.level == 0)
	      	{
			if (Npc_GetStateTime (self) > self.aivar[AIV_StateTime])
