...
func void B_AssessFightSound ()
{
	...
	&& ((victim.npctype == NPCTYPE_BL_AMBIENT)
	|| (Hlp_GetInstanceID(victim) == Hlp_GetInstanceID(STRF_1118_Addon_Patrick))
	|| (Hlp_GetInstanceID(victim) == Hlp_GetInstanceID(STRF_1119_Addon_Monty))
	|| (Hlp_GetInstanceID(victim) == Hlp_GetInstanceID(STRF_1120_Addon_Tonak))
	|| (Hlp_GetInstanceID(victim) == Hlp_GetInstanceID(STRF_1121_Addon_Telbor))
	|| (Hlp_GetInstanceID(victim) == Hlp_GetInstanceID(STRF_1122_Addon_Pardos)))
	{
		return;
	};

	if (C_NpcIsHero(victim))
	&& (self.guild == GIL_BDT)
	&& ((other.npctype == NPCTYPE_BL_AMBIENT)
	|| (Hlp_GetInstanceID(other) == Hlp_GetInstanceID(STRF_1118_Addon_Patrick))
	|| (Hlp_GetInstanceID(other) == Hlp_GetInstanceID(STRF_1119_Addon_Monty))
	|| (Hlp_GetInstanceID(other) == Hlp_GetInstanceID(STRF_1120_Addon_Tonak))
	|| (Hlp_GetInstanceID(other) == Hlp_GetInstanceID(STRF_1121_Addon_Telbor))
	|| (Hlp_GetInstanceID(other) == Hlp_GetInstanceID(STRF_1122_Addon_Pardos)))
	{
		return;
	};
