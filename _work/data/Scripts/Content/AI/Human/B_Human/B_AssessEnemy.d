...
{
	// EXIT IF...
	//Ich bin ein Ork (speziell: Nur-Shak) und der andere ist der Spieler UND Träger des Orkamulettes

	if ((self.guild > GIL_Seperator_Orc) && Npc_IsPlayer(other) && Orkamulett)
	{
		return FALSE;
	};
	...
	if ( (self.aivar[AIV_EnemyOverride] == TRUE) || (other.aivar[AIV_EnemyOverride] == TRUE) )
	  && ((other.guild < GIL_SEPERATOR_HUM) || (other.guild == GIL_SKELETON))
	{
		return FALSE;
	};
