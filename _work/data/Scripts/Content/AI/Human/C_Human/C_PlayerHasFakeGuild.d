...
func int C_PlayerHasFakeGuild (var C_NPC slf, var C_NPC oth)
{
	...
	else if slf.guild == oth.guild
	{
		...
		else if (oth.guild == GIL_BAD)
		&& ((Hlp_IsItem(itm, ITAR_DarkPupil))
		    ||(Hlp_IsItem(itm, ORE_ARMOR_M))
		    ||(Hlp_IsItem(itm, ITAR_DarkMage))
		    ||(Hlp_IsItem(itm, ITAR_Beliar)))
		{
		  return FALSE;
		}
		else if (Hlp_IsItem (itm, itar_vampir) == TRUE)
		{
			return FALSE;
		}
		else
