...
{	
	...
	// ------ Dunkelmagier ------
	if ((slf.guild == GIL_BAD) && (slf.aivar[AIV_MagicUser] == MAGIC_ALWAYS))
	  {
	    ...
	    if (slf.aivar[AIV_SelectSpell] <= 0) // 0 - Anfangen, 1-7 - Gegner ist der Spieler, 8-9 Gegner ist verrückter Mensch, 10-15 Gegner muss sterben (NSC)
	      { 
		if (NPC_isPlayer(oth)) //Spieler
		  {
		    slf.aivar[AIV_SelectSpell] = 1;
		  }
		else if ((oth.guild < GIL_SEPERATOR_HUM) && (!C_NpcHasAttackReasonToKill(slf))) //Mensch
		  {
		    if (oth.attribute[ATR_HITPOINTS] > 1)
		      {
			slf.aivar[AIV_SelectSpell] = 8;
		      }
		    else
		      {
			return FALSE; //Niederschlagen
		      };
		  }
		else
		  {
		    slf.aivar[AIV_SelectSpell] = 10;
		  };
	      };
	    if ((slf.aivar[AIV_SelectSpell] == 1) || (slf.aivar[AIV_SelectSpell] == 10))
	      {
		B_ReadySpell(slf, SPL_Blutopfer, SPL_Cost_Blutopfer);
	      };		
	    if ((slf.aivar[AIV_SelectSpell] == 2) || (slf.aivar[AIV_SelectSpell] == 8))
	      {
		B_ReadySpell(slf, SPL_SuckEnergy, SPL_Cost_SuckEnergy);
	      };
	    if ((slf.aivar[AIV_SelectSpell] == 3) || (slf.aivar[AIV_SelectSpell] == 4) || (slf.aivar[AIV_SelectSpell] == 12) || (slf.aivar[AIV_SelectSpell] == 11))
	      {
		B_ReadySpell(slf, SPL_Energyball, SPL_Cost_Energyball);
	      };
	    if ((slf.aivar[AIV_SelectSpell] == 5) || (slf.aivar[AIV_SelectSpell] == 13))
	      {
		B_ReadySpell(slf, SPL_Skull, SPL_Cost_Skull);
	      };
	    if ((slf.aivar[AIV_SelectSpell] == 6) || (slf.aivar[AIV_SelectSpell] == 14))
	      {
		B_ReadySpell(slf, SPL_BreathOfDeath, SPL_Cost_BreathOfDeath);
	      };
	    if ((slf.aivar[AIV_SelectSpell] == 7) || (slf.aivar[AIV_SelectSpell] == 9) || (slf.aivar[AIV_SelectSpell] == 15))
	      {
		slf.aivar[AIV_SelectSpell] = 0; //Neuer Zyklus
	      };
	    return TRUE;
	  };
	// ------ Magier ------
